...
export const editSynonyms = async (
  ...
  if (typeof synonymsList === "undefined") {
    return { status: "error", msg: "no synonymsList is sent" };
  }
  const synonymsSet = new Set(synonymsList);
  if (synonymsSet.size === 0) {
    return { status: "error", msg: "synonymsList contains no word" };
  }
